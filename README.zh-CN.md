...
- **Python 原生执行。** 26 个 `UnrealBridge*Library` 累计 1393 个 `UFUNCTION`，覆盖常见子系统；未封装的能力可直接通过 `unreal.*` 原生 API 调用。相较于固定工具列表的 MCP 方案与仅暴露单一 `call` 命令的反射协议，该设计在灵活性与结构性之间取得了折衷。所有关卡写操作均包裹于 `FScopedTransaction` 内，支持标准 Undo / Redo。
...
flowchart LR
    ...
    subgraph Host["Agent 主机"]
      ...
      Mani[("bridge_manifest.json<br/>26 个库 · 1393 UFUNCTION")]
    end
    ...
    CLI -- "UDP 探测<br/>默认本机；LAN 显式开启" --> Disc
...
用 UE 打开 `.uproject` 让它自动重建插件，或从命令行跑项目自带的 `Build.bat`。启动编辑器 —— 插件会在 `PostEngineInit` 拉起服务器，看到日志里出现 `LogUnrealBridge: Listening on 127.0.0.1:<端口>` 就算成功。端口由 OS 分配，客户端默认通过主机内多播与本机回环自动发现同一台机器上的编辑器，不需要手动配置；要发现其他主机上的编辑器，需显式传 `--discovery-scope=lan`。
...
- `--discovery-scope=local|lan` —— 安全默认值为 `local`（多播 TTL 0 + 回环）；`lan` 显式开启一跳 LAN 多播（也可用 `UNREAL_BRIDGE_DISCOVERY_SCOPE`）
...
`bridge.py list-editors` 默认只发主机内探测并列出响应的编辑器。只有明确需要扫描局域网时才使用 `bridge.py --discovery-scope=lan list-editors`。发现组必须是 IPv4 多播地址；单播 endpoint 应使用完整的直连身份元组。
...
| `UnrealBridgeBlueprintLibrary` | 蓝图全栈读写：类层级 / 变量 / 函数 / 组件 / 事件分发器；完整的 Blueprint Interface 资产与签名制作、实现事件/函数图、强类型接口调用与对象 Message 调用；支持可见/内部引脚名连线（`Target` ↔ `self`）；图谱的调用关系、执行流、引脚连接、节点搜索；20+ 类节点插入（Branch、Cast、循环、Delay、Timer、SpawnActor、MakeStruct 等）、引脚连接、节点坐标读写、对齐、注释框、AutoLayoutGraph；运行时调试 —— 断点增删查、`get_last_breakpoint_hit` 捕获函数 locals / params / return **加上**执行对象的 BP 类实例变量（带 `OwnerClass` 归属）、PIE node coverage；编译错误查询 |
| `UnrealBridgeAssetLibrary` | 资产关键字搜索（支持 include / exclude 词元）；安全复制现有资产到新的 `/Game` 路径且绝不覆盖；派生类查询；正向依赖与反向引用分析（含递归）；DataAsset / StaticMesh / SkeletalMesh / Texture / Sound 元信息；支持按索引、槽名或原子批量修改 StaticMesh/SkeletalMesh 默认材质（可用 `save=false` 临时预览）；目录树、重定向解析、批量 tag 与磁盘大小查询；**SearchableName 索引查询**（`find_assets_referencing_searchable_name` / `get_searchable_names_used_by_asset` / `list_searchable_name_values`）—— 编辑器右键 "Find References" 在 `GameplayTag` / `PrimaryAssetId` / 任意 USTRUCT 索引命名值上拿到的就是这套数据 |
...
| `UnrealBridgeMaterialLibrary` | 材质 / Master / Function 查询与事务式制作，包含依赖完整的本地图读取、隔离批次预检、连接节点的 fail-closed 删除、定向编译验证，以及 `UMaterialInstanceConstant` Layer Stack 的校验式快照 / 整体替换 / 严格根材质复制（UE 5.7+ 为真实实现，UE 5.3-5.6 为安全桩） |
...
1. **UDP 发现 v2**：端口 `9876`。默认 `local` scope 把带同一 request_id 的 `probe` 发往 TTL 0 的主机内多播 `239.255.42.99` 与本机回环 `127.0.0.1`，所以短生命周期 CLI 调用不会向 LAN 发出多播流量。只有 `--discovery-scope=lan`（或 `UNREAL_BRIDGE_DISCOVERY_SCOPE=lan`）才把多播 TTL 提升到 1，同时仍保留回环兜底。客户端拒绝畸形、旧版或字段不完整的响应，容忍 Windows UDP reset 通知，再按每次 Server 启动生成的 UUID 去重。每个编辑器返回 `protocol_version`、`instance_id`、PID、唯一 wire-canonical 工程路径、TCP endpoint、capabilities 与 token 指纹。既有六个 exact command 是最低必需 capability 集；`exact_editor_status` 单独广告且只由 status 命令要求，同时允许不重复的未来扩展项。Server 广告通配 TCP bind 时，客户端使用 UDP 响应源 IP。多编辑器通过 `SO_REUSEADDR` 共存。发现组必须是 IPv4 多播地址；直连单播使用冻结身份的完整直连元组。
