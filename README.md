...
flowchart LR
    ...
    subgraph Host["Agent host"]
      ...
      Mani[("bridge_manifest.json<br/>26 libs · 1393 UFUNCTIONs")]
    end
    ...
    CLI -- "UDP probes<br/>local by default; LAN opt-in" --> Disc
...
Open the `.uproject` and let UE rebuild the plugin automatically, or run the project's `Build.bat` from the command line. Launch the editor — the plugin starts the server at `PostEngineInit`. You're good once `LogUnrealBridge: Listening on 127.0.0.1:<port>` shows up in the log (the port is OS-assigned; the client finds a same-host editor via host-local multicast plus loopback — no manual config needed). Discovering editors on another host is explicit: pass `--discovery-scope=lan`.
...
- `--discovery-scope=local|lan` — `local` is the safe default (multicast TTL 0 + loopback); `lan` explicitly enables one-hop multicast (also env `UNREAL_BRIDGE_DISCOVERY_SCOPE`)
...
`bridge.py list-editors` sends a host-local probe and lists every editor that answered. Use `bridge.py --discovery-scope=lan list-editors` only when you intentionally want LAN-wide discovery. The discovery group must be an IPv4 multicast address; use the complete direct-mode tuple for unicast endpoints.
...
| `UnrealBridgeBlueprintLibrary` | Full-stack Blueprint read / write: class hierarchy / variables / functions / components / event dispatchers; complete Blueprint Interface asset/signature authoring, implementation events/function graphs, strongly typed calls and object-message calls; friendly/internal pin-name wiring (`Target` ↔ `self`); graph call relationships, exec flow, pin connections, node search; 20+ node-type insertion (Branch, Cast, loops, Delay, Timer, SpawnActor, MakeStruct, …), pin connect, node-coordinate read / write, alignment, comment boxes, AutoLayoutGraph; runtime debug — set/list/clear breakpoints, `get_last_breakpoint_hit` captures function locals/params/return **plus** the executing object's BP-class instance variables (tagged with `OwnerClass`), PIE node coverage; compile-error query |
| `UnrealBridgeAssetLibrary` | Asset keyword search (include / exclude tokens); safe non-overwriting duplication to a new `/Game` path; derived-class query; forward-dependency and reverse-reference analysis (recursive); DataAsset / StaticMesh / SkeletalMesh / Texture / Sound metadata; transactional StaticMesh/SkeletalMesh default-material authoring by index, slot name, or atomic batch (`save=false` previews supported); folder tree, redirector resolution, batch tag and disk-size query; **SearchableName index queries** (`find_assets_referencing_searchable_name` / `get_searchable_names_used_by_asset` / `list_searchable_name_values`) — the data backing the editor's right-click "Find References" on a `GameplayTag` / `PrimaryAssetId` / any USTRUCT-keyed named value |
...
| `UnrealBridgeMaterialLibrary` | Material/master/function introspection and transactional authoring, including dependency-complete local graph reads, isolated batch preflight, fail-closed connected-node deletion, targeted compile validation, and validated `UMaterialInstanceConstant` layer-stack snapshot/full replacement/strict-root copy (UE 5.7+ real implementation; UE 5.3-5.6 safe stubs) |
...
1. **UDP discovery v2** on port `9876`. The default `local` scope sends the same request-id-bearing `probe` to host-local multicast `239.255.42.99` with TTL 0 and to loopback `127.0.0.1`; short-lived CLI calls therefore do not emit LAN multicast traffic. `--discovery-scope=lan` (or `UNREAL_BRIDGE_DISCOVERY_SCOPE=lan`) is the explicit opt-in that raises multicast TTL to 1 while retaining the loopback fallback. The client rejects malformed/legacy/incomplete replies, tolerates Windows UDP reset notifications, then de-duplicates by Server-start UUID. Every editor replies with `protocol_version`, `instance_id`, PID, one wire-canonical project-path string, TCP endpoint, capabilities, and token fingerprint. The six pre-existing exact commands are the minimum required capability set; `exact_editor_status` is advertised separately and required only by the status command, while unique future capabilities remain allowed. When a Server advertises a wildcard TCP bind, the client connects to the UDP response source IP. Multiple editors coexist via `SO_REUSEADDR`. Discovery groups must be IPv4 multicast addresses; direct unicast uses the identity-frozen direct-mode tuple.
...
Status  :  {"command":"exact_editor_status","expected":{...},"request":{}}  →  cached health
```
...
- CLI discovery is host-local by default. LAN multicast requires the explicit `--discovery-scope=lan` opt-in.
