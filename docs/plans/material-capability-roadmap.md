...
最后更新：2026-04-24（v0.3 — M3 7/9 模板（含 VFX 双模板合并计数）、M4 4/5 模板、M5 10 条规则 + auto_fix；handoff list item 1/2/3/4/5 本次交付）
...
| M5 Lint / 自动修复 | 🟡 10 / 13 规则 + auto_fix | **已交付**：analyze_material 聚合 + M5-2 预算、M5-3 不可达节点、M5-4 重复 TextureSample、M5-5 SamplerSource 混用、M5-6 静态开关误用 (dead constant alpha + boolean-intent ScalarParameter)、M5-8 ShadingModel↔wiring、M5-9 MI chain depth + StaticSwitch 覆盖风险、M5-10 texture compression↔SamplerType 合规、M5-11 Custom body trivia (snippet-dispatch 豁免)、M5-12 Custom 内部 SceneTextureLookup 绕过分析；auto_fix_material 支持 `drop_unused` + `samplersource_share`. **未交付**：M5-7 feature level / quality switch 缺失、M5-13 Custom SM-only intrinsic 缺 FeatureLevelSwitch、auto_fix 里的 `static_switch_conversion` / `inline_trivial_custom` |
...
5. ✅ ~~**M5-6 静态开关误用**~~ (已交付，commit dd844ea — dead Constant alpha + boolean-intent ScalarParameter 两种模式，双 info-level)

剩余（**按下次 session 推荐顺序**排列，仍互不依赖）：

6. **M3-7 Layered 材质层框架** — `MaterialAttributeLayers` + 3-4 个 `MF_Layer_*` MaterialFunction (Metal / Fabric / Dirt)。体量最大（300-500 行 Python），但覆盖 AAA 项目的"层级材质"workflow。
7. **M3-5 Weapon_Hero** — 建立在 M3-2 基础上 + Parallax Occlusion（POM）HLSL snippet（~40 行 HLSL）+ Curve Atlas 驱动的脉冲发光 + 第二套 UV (Emissive-only)。需要先加 `BridgePOMRayMarch` snippet；无 POM 版本可简化为"M3-2 + 脉冲发光 + 双 UV"，~300 行 Python。
8. **M5-7 Feature/Quality switch 缺失检查** — 对含 `Custom` 节点或 >4 次 TextureSample 的图，若外围没 `FeatureLevelSwitch` / `QualitySwitch` 包装，出 info。约 80 行 C++。
9. **M5-13 Custom SM-only intrinsic 检查** — Custom 节点 Code 内含 `ddx_fine` / 部分 bit ops 但外围没 `FeatureLevelSwitch` → warning。正则扫字符串，约 40 行 C++。
...
期望：全部 `warnings=0`，无 exception。info-level finding 可接受（主要是 M5-10 的 engine placeholder 纹理和 M5-9 的 MI chain depth=1）。如果 warning 冒出，大概率是引擎版本升级或某个默认 MI 参数漂移，优先查 `analyze_material` 的 detail 字段。
