...
| 9 | **GameplayAbility 图编辑** (M1/3 交付) | 大 | 中-高 | 原只读 CDO。**M1 交付 2026-04-20**：`UnrealBridgeGameplayAbilityLibrary` 加 8 个 CDO 写 UFUNCTION — `set_ability_tag_container`（10 个 FGameplayTagContainer 统一）/ `set_ability_instancing_policy` / `set_ability_net_execution_policy` / `set_ability_cost` / `set_ability_cooldown` / `add_ability_trigger` / `remove_ability_trigger_by_tag` / `clear_ability_triggers`。剩余：M2 `ActivateAbility` 图节点写（UAbilityTask_\* async pin 展开是硬点）；M3 `CreateGameplayAbilityBlueprint` 工厂。 |
