...
| 9 | **GameplayAbility 图编辑** (M1+M2/3 交付) | 大 | 中-高 | 原只读 CDO。**M1+M2 交付 2026-04-20**：M1 八个 CDO 写 — `set_ability_tag_container`（10 个 FGameplayTagContainer 统一）/ `set_ability_instancing_policy` / `set_ability_net_execution_policy` / `set_ability_cost` / `set_ability_cooldown` / `add_ability_trigger` / `remove_ability_trigger_by_tag` / `clear_ability_triggers`。M2 四个图节点写 — `list_ability_task_classes` / `list_ability_task_factories`（发现）/ `add_ability_task_node`（spawn `UK2Node_LatentAbilityCall`，自动展开 async delegate pins：EventReceived/Payload/OnSuccess/OnInterrupted…）/ `add_ability_call_function_node`（GA 方法 CallFunction，带 DisplayName/ScriptName fallback）。剩余：M3 `CreateGameplayAbilityBlueprint` 工厂。 |
