...
| **Motion Matching — PoseSearch** | `${CLAUDE_SKILL_DIR}/references/bridge-pose-search-api.md` | PoseSearchSchema (PSS) info / channels and PoseSearchDatabase (PSD) info / animation entry CRUD / async index lifecycle. **Read before any PSS / PSD read or write** — the underlying UPROPERTYs (`DatabaseAnimationAssets`, `Channels`) are `private:` and unreachable via `get_editor_property`; this library is the only path. Includes the `wait-pose-index` CLI for sequencing post-edit verifications. |
| **Motion Matching — Chooser** | `${CLAUDE_SKILL_DIR}/references/bridge-chooser-api.md` | ChooserTable (CHT) info / column listing / row CRUD / typed column adds / cell raw T3D writes / `evaluate_chooser_with_context_object`. **Read before any CHT read or write** — `ResultsStructs` / `DisabledRows` / `NestedChoosers` / `FallbackResult` are `private:` and unreachable via `get_editor_property`; this library is the only path. Covers NestedChooser `:Name` subobject path convention and the `matched_row=-1` caveat in evaluation. |
